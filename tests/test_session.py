...
from aesc.metadata import load_metadata
...
def isolated_share_dir(monkeypatch, tmp_path: Path) -> Path:
    ...
    monkeypatch.setattr("aesc.share.get_share_dir", _get_share_dir)
    monkeypatch.setattr("aesc.metadata.get_share_dir", _get_share_dir)
    return share_dir

...
from aesc.tools.file.glob import MAX_MATCHES, Glob, Params
...
@pytest.mark.asyncio
async def test_glob_allowed_system_paths(glob_tool: Glob):
    """Test that glob allows searching in allowed system paths like /usr/share."""
    # Skip on Windows
    if platform.system() == "Windows":
        pytest.skip("System paths test only applies to Linux")

    # Test /opt - should be allowed (one of the allowed system paths)
    # This directory usually exists but may be empty, which is OK
    result = await glob_tool(Params(pattern="*", directory="/opt"))

    # Should not fail with "outside working directory" error
    # May return ToolOk (with or without matches) or ToolError for other reasons
    if isinstance(result, ToolError):
        # Should not be the "outside working directory" error
        assert "outside the working directory" not in result.message.lower()


@pytest.mark.asyncio
async def test_glob_disallowed_system_paths(glob_tool: Glob):
    """Test that glob blocks searching in non-allowed system paths."""
    # Skip on Windows
    if platform.system() == "Windows":
        pytest.skip("System paths test only applies to Linux")

    # Test /etc - should NOT be allowed (not in the allowed paths list)
    result = await glob_tool(Params(pattern="*", directory="/etc"))

    assert isinstance(result, ToolError)
    assert "outside the working directory" in result.message.lower()

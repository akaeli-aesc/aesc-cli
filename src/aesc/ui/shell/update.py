...
BASE_URL = "https://github.com/akaeli-aesc/aesc-cli/releases/download"
...
async def _do_update(*, print: bool, check_only: bool) -> UpdateResult:
    ...
    async with new_client_session() as session:
        ...
        filename = f"aesc-{latest_version}-{target}.tar.gz"
        download_url = f"{BASE_URL}/{latest_version}/{filename}"

        with tempfile.TemporaryDirectory(prefix="aesc-") as tmpdir:
            ...
            try:
                ...
                for root, _, files in os.walk(tmpdir):
                    if "aesc" in files:
                        binary_path = os.path.join(root, "aesc")
                        break
                if not binary_path:
                    logger.error("Binary aesc not found in archive.")
                    _print("[red]Binary aesc not found in archive.[/red]")
                    return UpdateResult.FAILED
            except Exception:
    ...
    _print("[yellow]Restart AESC to use the new version.[/yellow]")

...
from aesc.provider import Message, TextPart, ToolError, ToolOk
from aesc.soul import StatusSnapshot
from aesc.ui.shell.console import console
from aesc.ui.shell.prompt import PROMPT_SYMBOL
from aesc.utils.message import message_extract_text, message_stringify
from aesc.wire import Wire
from aesc.wire.message import ContentPart, StepBegin, ToolCall, ToolResult
...
async def replay_recent_history(history: Sequence[Message]) -> None:
    ...
    Uses the Textual visualizer for consistent display.
    """
    ...
    for run in runs:
        ...
        ui_task = asyncio.create_task(
            visualize_for_textual(
                wire.ui_side,
                chat_app=None,  # Will just skip if no chat app
                initial_status=StatusSnapshot(context_usage=0.0),
            )
        )

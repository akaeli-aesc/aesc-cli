...
def _listen_for_keyboard_unix(
    ...
    try:
        if not os.isatty(sys.stdin.fileno()):
            # Not a terminal (e.g., Docker without -it, piped input)
    ...
    # Set up raw mode with proper error handling
    fd = None
    oldterm = None
    try:
        fd = sys.stdin.fileno()
        ...
        # Failed to set up raw mode - terminal not available or invalid
        return
    ...
    try:
        ...
        # Terminal disconnected or invalid during read
        pass
    finally:
        # restore the terminal settings
        if oldterm is not None and fd is not None:
            try:
                termios.tcsetattr(fd, termios.TCSAFLUSH, oldterm)
            except (OSError, termios.error):
                pass  # Terminal no longer available

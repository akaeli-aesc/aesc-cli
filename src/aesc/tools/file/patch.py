...
class PatchFile(CallableTool2[Params]):
    ...
    def _validate_path(self, path: Path) -> ToolError | None:
        ...
        return validate_path_or_error(path, self._work_dir, operation="patch")
    ...
    async def __call__(self, params: Params) -> ToolReturnType:
        try:
            ...
            return ToolError(
                message=f"Permission denied patching: {params.path}",
                brief="Permission denied",
            )
        except OSError as e:
            return ToolError(
                message=f"Failed to patch {params.path}: {e}",
                brief="I/O error",
            )

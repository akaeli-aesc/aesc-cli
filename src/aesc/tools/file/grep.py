...
def _find_existing_rg(bin_name: str) -> Path | None:
    ...
    local_dep = Path(aesc.__file__).parent / "deps" / "bin" / bin_name
...
async def _download_and_install_rg(bin_name: str) -> Path:
    ...
    # GitHub releases URL format: /download/{version}/{filename}
    ...
    async with new_client_session() as session:
        with tempfile.TemporaryDirectory(prefix="aesc-rg-") as tmpdir:
...
class Grep(CallableTool2[Params]):
    ...
    async def __call__(self, params: Params) -> ToolReturnType:
        try:
            ...
            return ToolError(
                message=f"I/O error during grep: {e}",
                brief="I/O error",
            )

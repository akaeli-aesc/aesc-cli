...
from aesc.provider import ContentPart
from aesc.utils.logging import logger
from aesc.wire import Wire, WireUISide
from aesc.wire.message import WireMessage

if TYPE_CHECKING:
    from aesc.llm import LLM, ModelCapability
...
async def run_soul(
    ...
    try:
        ...
        wire.shutdown()
        try:
            await asyncio.wait_for(ui_task, timeout=0.05)
        except asyncio.QueueShutDown:
            logger.debug("UI loop shut down")
        except TimeoutError:
            logger.warning("UI loop timed out, cancelling")
            ui_task.cancel()
            try:
                await ui_task
            except asyncio.CancelledError:

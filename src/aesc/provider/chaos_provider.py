...
class ChaosProvider(OpenAIProvider):
    """OpenAIProvider with chaos error injection."""
    ...
    def _monkey_patch_client(self):
        ...
        # AsyncOpenAI uses a private _client (httpx.AsyncClient)
        # We need to reach into it to patch the transport
        original_transport = self.client._client._transport
        chaos_transport = ChaosTransport(original_transport, self._chaos_config)
        self.client._client._transport = chaos_transport
